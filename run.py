...
class Run():
    ...
    def sum(self, inputs, outputs, other_name = 'or'):
        ...
        for root, _, files in os.walk(inputs[0]):
            ...
            pars = tqdm(files)
            for file in pars:
                pars.set_description(f'Calculating SHA256 Dir {d}')
                file_path = os.path.join(root, file)
                with open(file_path, 'rb') as f:
                    file_hash = hashlib.sha256(f.read()).hexdigest()
                # 检查文件在文件夹2中是否存在，并且SHA256哈希值是否相同
                is_exist = False
                for input in inputs[1:]:
                    if os.path.exists(os.path.join(input + r'\\' + d, file)) and \
                    hashlib.sha256(open(os.path.join(input + r'\\' + d, file), 'rb').read()).hexdigest() == file_hash:
                        is_exist = True
                        break
                if is_exist:
                    same_files[file] = d
        ...
        for file, label in tqdm(same_files.items(), desc='Copying files'):
            ...
            src_file = os.path.join(inputs[1] + '\\' + label, file)
...
if __name__ == "__main__":
    model_path_list = [r'checkpoint\ResNet18_Epoch_82.pth', 
                       r'checkpoint\ResNet18_Epoch_65.pth']
    output_list = [r'output', r'output.old']
    ...
    run = Run(
                ...
                r'input', \
                output_list[0])
    run.sum(output_list, r'output.sum')
